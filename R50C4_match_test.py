...
# 定义ResNet50特征提取器，保留到C4阶段
class ResNet50C4Backbone(nn.Module):
    def __init__(self):
        super().__init__()
...
model = ResNet50C4Backbone().to(device)
model.eval()
...
# 生成掩码
mask = generate_mask_from_similarity(sim_map_resized, threshold=0.35)
